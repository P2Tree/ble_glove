...
#define HAL_KEY_DEBOUNCE_VALUE  1
...
#define HAL_KEY_SW_7_ICTLBIT  BV(5) /* P0IEN - P1.5 enable/disable bit */
#define HAL_KEY_SW_7_PXIFG    P1IFG /* Interrupt flag at source */
   
/* HAL_CHARGE is at P1.2 */
#define HAL_CHARGE_PORT   P1
#define HAL_CHARGE_BIT    BV(2)
#define HAL_CHARGE_SEL    P1SEL
#define HAL_CHARGE_DIR    P1DIR

#define HAL_CHARGE_EDGEBIT  BV(2)
#define HAL_CHARGE_EDGE     HAL_KEY_FALLING_EDGE

#define HAL_CHARGE_IEN      IEN2  /* CPU interrupt mask register */
#define HAL_CHARGE_IENBIT   BV(4) /* Mask bit for all of Port_1 */
#define HAL_CHARGE_ICTL     P1IEN /* Port Interrupt Control register */
#define HAL_CHARGE_ICTLBIT  BV(2) /* P0IEN - P1.2 enable/disable bit */
#define HAL_CHARGE_PXIFG    P1IFG /* Interrupt flag at source */

#elif defined ( BACKPACK )
...
/* edge interrupt */
#define HAL_KEY_SW_6_EDGEBIT  BV(0)
#define HAL_KEY_SW_6_EDGE     HAL_KEY_RASING_EDGE
...
void HalKeyInit( void )
{
  ...
  HAL_CHARGE_SEL &= ~(HAL_CHARGE_BIT);    /* Set pin function to GPIO */
  HAL_CHARGE_DIR &= ~(HAL_CHARGE_BIT);    /* Set pin direction to Input */
#elif defined ( BACKPACK )
...
void HalKeyConfig (bool interruptEnable, halKeyCBack_t cback)
{
  ...
  if (Hal_KeyIntEnable)
  {
    ...
    /* HAL_CHARGE*/
    /* Rising/Falling edge configuratinn */
    PICTL &= ~(HAL_CHARGE_EDGEBIT);    /* Clear the edge bit */
    /* For falling edge, the bit must be set. */
  #if (HAL_CHARGE_EDGE == HAL_KEY_FALLING_EDGE)
    PICTL |= HAL_CHARGE_EDGEBIT;
  #endif
    
    HAL_CHARGE_ICTL |= HAL_CHARGE_ICTLBIT;
    HAL_CHARGE_IEN |= HAL_CHARGE_IENBIT;
    HAL_CHARGE_PXIFG = ~(HAL_CHARGE_BIT);
  ...
  else    /* Interrupts NOT enabled */
  {
    ...
    HAL_CHARGE_ICTL &= ~(HAL_CHARGE_ICTLBIT); /* don't generate interrupt */
    HAL_CHARGE_IEN &= ~(HAL_CHARGE_IENBIT);   /* Clear interrupt enable bit */
#elif defined ( BACKPACK )
...
uint8 HalKeyRead ( void )
{
  ...
  if ( !(HAL_CHARGE_PORT & HAL_CHARGE_BIT))    /* Key is active low */
  {
    keys |= HAL_CHARGE;
  }
#elif defined ( BACKPACK )
...
bool HalKeyPressing ( uint8 keys )
{
  if (keys & HalKeyRead())
    return TRUE;
  else
...
void HalKeyPoll (void)
{
  ...
  /* �ⲿ����Ҫ���������������� */
#if defined ( GLOVE )
  if (!(HAL_KEY_SW_6_PORT & HAL_KEY_SW_6_BIT))    /* Key is pull to high, active is low*/
  {
    keys |= HAL_KEY_SW_6;
  }
  if (!(HAL_KEY_SW_7_PORT & HAL_KEY_SW_7_BIT))    /* Key is pull to high, active is low*/
  {
    keys |= HAL_KEY_SW_7;
  }
  if (!(HAL_CHARGE_PORT & HAL_CHARGE_BIT))    /* Key is pull to high, active is low*/
  {
    keys |= HAL_CHARGE;
  }
  ...
  */
  ...
  if (!Hal_KeyIntEnable)  // Interrupts not enabled
  {
    if (keys == halKeySavedKeys)
    {
      keyPressTime++;
      // Polling function called every 100ms
      if ( keyPressTime == 20)  // 20 * 100ms = 2s
      {
        sendkeys = keys | HAL_KEY_LONG;
    ...
    else
    {
      if ( keyPressTime < 5)
      {
        sendkeys = keys | HAL_KEY_SHORT;
        (pHalKeyProcessFunction) ( sendkeys, HAL_KEY_STATE_NORMAL);
      }
      notify = 1;
    }
  }
  else  // Key interrupt enabled
  {
    /* Key interrupt handled here */
    keyPressTime = 1; // begin to count for long press detection
...
#if !defined ( GLOVE ) && !defined ( BACKPACK )
/**************************************************************************************************
...
void halProcessKeyInterrupt (void)
{
  ...
  if (HAL_CHARGE_PXIFG & HAL_CHARGE_BIT)  /* Interrupt Flag has been set */
  {
    HAL_CHARGE_PXIFG = ~(HAL_CHARGE_BIT); /* Clear Interrupt Flag */
    valid = TRUE;
  }
#elif defined ( BACKPACK )
...
{
  /* WARNING: This interrupt function is ONLY used for PORT1 */
  HAL_ENTER_ISR();
#if defined ( GLOVE )
